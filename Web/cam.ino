...
// const char* ssid = "SK_WiFiGIGA8DE3_2.4G";
// const char* password = "2003031452";
...
void loop() {
      ...
      if (client.available()) { // if there's bytes to read from the client,
        ...
        if (c == '\n') { // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            client.println("<img id=\"stream\" src=\"http://192.168.0.2:81/stream\" crossorigin >");
            client.println("<img id=\"stream\" src=\"http://192.168.214.71:81/stream\">");
  ...
  /// 시간 비교  
   ...
   if (isTimeInRange(serverYear, serverMonth, serverDay, serverHours, serverMinutes, startYear, startMonth, startDay, startHour, startMinute, endYear, endMonth, endDay, endHour, endMinute)) {
    ...
    Serial.println("isTimeInRange = True");
      // NIGHT 알람 설정 
    if (
